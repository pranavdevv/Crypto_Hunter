...
_Hackathon Context: We are using the **Tambo.co SDK** as the core engine. Anomalies are generated by the AI agent dynamically swapping components in response to game state._
...
- **UI Layout**:
  ...
  - _(Removed System Log Sidebar per user request)_

### 2. The Anomaly Engine (The "Tambo Agent")

- **Concept**: A "System Agent" running in the background via Tambo SDK.
- **Hide & Seek**: The Chat UI is hidden. The player only sees the _effects_ (glitches).
- **Mechanism**:
...
```mermaid
graph LR
    Buy["🟢 Standard<br/>Buy Button"]
    Sell["🔴 Standard<br/>Sell Button"]
    Graph["📊 Standard<br/>Crypto Chart"]

    Buy --> BuyRed["🔴 Red<br/>Danger Color"]
    Buy --> BuyTypo["📝 Typo<br/>Bad Font"]
    Buy --> BuyJitter["⚡ Jitter<br/>Shaking"]
    Buy --> BuyGhost["👻 Ghost<br/>Disappears"]
    Buy --> BuyBounce["🏀 Bounce<br/>Moves Away"]
    Buy --> BuyRev["🔄 Reverse<br/>Executes SELL"]

    Sell --> SellGreen["🟢 Green<br/>Safe Color"]
    Sell --> SellTypo["📝 Typo<br/>Bad Font"]
    Sell --> SellRev["🔄 Reverse<br/>Executes BUY"]

    Graph --> GNoAxis["❌ NoAxis<br/>No Labels"]
    Graph --> GNoGrid["⬜ NoGrid<br/>No Depth"]
    Graph --> GNeon["💜 Neon<br/>Neon Color"]
    Graph --> GFlat["📉 Flatline<br/>Fake Crash"]
...
- **Objective**: Turn Start Balance ($10,000) into Goal (**$25,000**).
- **Tick Rate**: Prices update every **2 seconds** (fast-paced gameplay).
- **Win Condition**: Reach $25,000 balance.
- **Fail State 1**: Balance hits $0 (Bankruptcy).
- **Fail State 2**: `activeAnomalies.length` >= 5 (System Failure).
...
4.  **Tambo Integration / Dynamic Swapping Risks**:
    - **Prop Incompatibility**: If a Glitch Variant doesn't accept _exactly_ the same props as the Normal component (e.g., missing an `onClick` handler), the game logic breaks, not just the UI.
      - _Fix_: **Strict Prop Forwarding**. The `TamboComponent` wrapper will force-feed all properties (`...props`) to the child. If the child ignores them, it's a bug in the child, but the app won't crash.
    - **State Reset**: Swapping a component (`<ComponentA />` -> `<ComponentB />`) forces a re-mount. Any internal state (like "isHovered" or text input focus) will be lost instantly, feeling jerky.
      - _Fix_: **Lifted State**. We moved critical data (like "Coins Amount to Buy") out of the button and into the main Game State. The button becomes just a "dumb" trigger.
    - **Context Loss**: Swapped components might lose access to React Context.
      - _Fix_: **Context Bridging**. The Anomaly must be rendered _inside_ the main Providers, not outside.
    - **Layout Shift (CLS)**: If the Glitch variant is 1px larger, the whole UI might jump.
      - _Fix_: **Strict Sizing**. All variants must have `width: 100%; height: 100%` of their container.
